...
export default {
  ...
  computed: {
    filteredPosts() {
      if (this.tabs) {
        return this.$store.getters.POSTS_FILTERED_BY_ANSWERS.filter(e => e.retirement_tool_category.indexOf(this.section.slug) >-1 && e.retirement_tool_timeframe.indexOf(this.tab.slug) > -1).sort(this.sortTerms)
      }
